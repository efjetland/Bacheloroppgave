...
class Loggerapp(tk.Tk):
    ...
    def changeView(self, windowName):
        if windowName in self.windows.keys():
            self.windows[windowName].tkraise()
        else:
            print("Window {} not found".format(windowName))

class StartWindow(tk.Frame):

    def __init__(self, parent, windowController):
        tk.Frame.__init__(self, parent)
        tk.Label(self, text="Initializing screens... Loading data...", font=LARGE_FONT).grid(sticky='SN')
...
class MainWindow(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        stopButton = tk.Button(buttonPanel, relief="flat", bg=BACKGROUND_COLOR, activebackground=BACKGROUND_COLOR, image=self.stopButtonImage, borderwidth=0, highlightthickness=0, padx=0, pady=0, command=self.stopButtonAction)
        stopButton.grid(row=0, column=1, sticky="S", padx=50, pady=10)
    ...
    def generateSettings(self, panel):
        ...
        for index, (key, value) in enumerate(sensors.items()):
            ...
            sensorToggle = tk.Button(panel, text=key, bg='green', width=20, relief='flat', overrelief='flat')
            action = partial(self.toggleButton, key)
            sensorToggle["command"] = action
            sensorToggle.configure(state='normal', relief='flat', bg='green')
            sensorToggle.grid(row=index, column=0, padx=10, pady=5)

            self.sensorButtons[key] = sensorToggle

    def toggleButton(self, name):
        print name
        print self.sensorButtons[name]['bg']
        if self.sensorButtons[name]['bg'] == 'green':
            self.sensorButtons[name]['bg'] = 'red'
            ...
            self.sensorButtons[name]['bg'] = 'green'
            if not name in activesensors:
                activesensors.append(name)
                for i, (key, value) in enumerate(sensors.items()):
                    if key == name:
                        x = timestamps[0:len(timestamps)]
                        y = value[0:len(timestamps)]
...
while isRunning:
    if status == RUNNING:
        ...
        app.windows["mainWindow"].updateGraph()
    app.update()
